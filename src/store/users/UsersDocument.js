...
const mutations = {
  ...
  UPDATE_DOCUMENT(state, getDocument) {
    ...
    while (
      state.DocumentArr.map((e) => e.id).includes(parseInt(checkOverlapID)) ===
      true
    ) {
      checkOverlapID++;
    }
    state.DocumentArr = state.DocumentArr.filter(
      (e) => e.id !== getDocument.id
    );
    getDocument.id = checkOverlapID;
    getDocument = makeDocument(getDocument);
    state.DocumentArr.push(getDocument);
    state.Document = initDocument();
  },
  ...
  ADD_DOCUMENT(state, getDocument) {
    state.DocumentArr.unshift(getDocument);
    state.Document = initDocument();
  },
  ...
  WRITER_INPUT(state, data) {
    for (let element of data) {
      state.DocumentArr.find(
        (D) => D.name === state.want_Document_Name
      ).documentWriter.push(makeWriterDocument(element));
    }
    state.Document = initDocument();
