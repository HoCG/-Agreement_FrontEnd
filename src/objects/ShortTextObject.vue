<template>
    <div class="ShortTextObjectArea">
        ...
        <svg
            ...
            v-bind:id="'ShortTextDeleteBtn'+getSTData.id"
            class="CloseBtn"
            width="20"
            height="20"
            viewBox="0 0 20 20"
    ...
    export default {
        ...
        methods: {
            myFunction() {
                ...
                const NewElementDiv = document.getElementById(this.getSTData.htmlID);
                NewElementDiv.style.left = this.getSTData.x + "px";
                NewElementDiv.style.top = this.getSTData.y + "px";
                this.makingFirstClickObject(this.getSTData.htmlID);
            ...
            appendIntoPDFPage(getElement, currentX, currentY) {
                ...
                for (let i = 1; i <= this.$store.state.PDFInfo.PDFPageInfo; i++) {
                    ...
                    if (currentY >= appendY1 && currentY <= appendY2) {
                        ...
                        this.getSTData.y = currentY - appendY1 - parseInt(
                            computed_Object_Style.height,
                            10
                        ) / 2;
                        this.getSTData.x = currentX;
            ...
            makingDragEvent(getElement) {
                ...
                let currentY = 0;
                let self = this;
                let ShortTextDeleteBtn = document.getElementById(
                    "ShortTextDeleteBtn" + this.getSTData.id
                );
                //self.showObjectMenu(); getElement.removeEventListener('mousemove')
                getElement.onmouseover = function (event) {
                    event.stopPropagation();
                    ShortTextDeleteBtn.style.display = "block";
                }
                getElement.onmouseout = function (event) {
                    event.stopPropagation();
                    ShortTextDeleteBtn.style.display = "none";
                }
